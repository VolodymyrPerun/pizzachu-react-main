...
export const getCommentsFromDB = (
  productId, pageSize, currentPage) => async dispatch => {

  try {
    dispatch(setIsLoading(true))
    dispatch(setCurrentPage(currentPage))

    const commentsInfo = await commentAPI.getAllComments(productId, pageSize,
      currentPage)
...
export const sendComment = (
  productId, data, pageSize, currentPage) => async dispatch => {
...
export const editChosenComment = (
  comment_id, data, productId, pageSize, currentPage) => async dispatch => {
  ...
  try {
    ...
    if (e.response.data.code === CUSTOM_ERRORS[4012].code) {
      dispatch(refreshUserToken())
      dispatch(
        editChosenComment(comment_id, data, productId, pageSize, currentPage))
    }
  }
}

export const deleteChosenComment = (
  comment_id, productId, pageSize, currentPage) => async dispatch => {
  ...
  try {
    ...
    if (e.response.data.code === CUSTOM_ERRORS[4012].code) {
      dispatch(refreshUserToken())
      dispatch(
        deleteChosenComment(comment_id, productId, pageSize, currentPage))
    }
  }
}
